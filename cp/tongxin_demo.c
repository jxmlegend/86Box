...
#include "CRC_ALG.h"
#include "cJSON.h"
#include "xml_haudongtiao_demo.h"
...
static __s32 Uart_Send(unsigned char * packet,int len)
{
	...
	f_uart = eLIBs_fopen("b:\\BUS\\UART1", "r+");
	if(f_uart == (ES_FILE *)0){
...
__s32 xml_Lamp_Sent_gzq(H_WIN handle, char * cha,int delay)
{
    ...
    wndDpt = (SCR_ScrollingButtonDescriptor *)GUI_WinGetAttr(handle);
	...
	cJSON_AddStringToObject(root, "dev_id", wndDpt->dev_id);
	cJSON_AddStringToObject(root, "dev_type", wndDpt->dev_type);
	cJSON_AddNumberToObject(root, "key_code", wndDpt->key_code);
	cJSON_AddNumberToObject(root, "key_status", wndDpt->buttonStatus);
	if(cha != NULL) {
		cJSON_AddStringToObject(root, "bin_data", cha);
	}
	//uart_send_data_frame(root, delay);

	out = cJSON_Print(root);
	cJSON_Delete(root);
	uart_frame = esMEMS_Malloc(0, sizeof(uart_frame_t));
	if(uart_frame != NULL) {
		memset(uart_frame, 0, sizeof(uart_frame_t));
		sprintf(uart_frame->data, "%s\r\n\r\n", out);
		uart_frame->len = strlen(uart_frame->data);
	}
	Uart_Send(uart_frame->data,uart_frame->len);
	free(uart_frame);
	free(out);
...
{					
	...
	if(GPIO_PIN==1){
		...
		xml_Lamp_Sent_gzq(handle, datas,SLIB_atoi(delay));
	}
}
...
static int Chang_Button_State_gzq(H_WIN  Gbutton_handle, int button_status)
{
	__gui_msg_t  msgsend;
	
	msgsend.dwAddData2 = button_status;
	msgsend.id = GUI_MSG_USER_GESTURE_CHANGE_ZHUANGTAI;
	msgsend.h_deswin = Gbutton_handle;
	GUI_PostMessage(&msgsend);
	return EPDK_OK;
...
void parse_uart_msg(char *buffer, int length)
{
    ...
    if (!root) {  
        eLIBs_printf("Error before: [%s]\n",cJSON_GetErrorPtr());
		return;
    }
    dev_id = cJSON_GetObjectItem( root , "dev_id");
	if(!dev_id) {
		eLIBs_printf("no dev_id\n");
		return;
	}
	...
	eLIBs_printf("dev_id = %s, dev_type = %s, key_code = %d, key_status = %d\n", \
		dev_id->valuestring, dev_type->valuestring, key_code->valueint, key_status->valueint);

	handle = XML_haudongtiao_WinHandle_Search_gzq(dev_id->valuestring, dev_type->valuestring, key_code->valueint);
	if(handle != NULL) {
		Chang_Button_State_gzq(handle->need_handle, key_status->valueint);
	}
			...
			if(uart_msg.pos < sizeof(uart_msg.msg_buf)) {
				 ...
				 if(c == '\n' && uart_msg.pos >= 6) {
					 if(strncmp((char*)&uart_msg.msg_buf[uart_msg.pos - 4], "\r\n\r\n", 4) == 0) {
						 ...
						 //eLIBs_printf("%s\n", uart_msg.msg_buf);
						 memset(uart_msg.msg_buf, 0, sizeof(uart_msg.msg_buf));
						 uart_msg.pos = 0;
						 continue;
					 }
				 }
