...
void * picswap_quadrant_begin(__u32 clockhands[CLOCKHANDS], int offset, int radius)
{
    ...
    handler->clockhands[0] = dsk_Systheme_open(clockhands[0]);
    handler->handdata[0] = dsk_Systheme_hdl2buf(handler->clockhands[0]);
    ...
    for (i = 1; i < CLOCKHANDS; i++) {
        handler->clockhands[i] = dsk_Systheme_open(clockhands[i]);
        handler->handdata[i] = dsk_Systheme_hdl2buf(handler->clockhands[i]);
