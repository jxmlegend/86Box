...
void button_click_handle(SCR_ScrollingButtonDescriptor * wndDpt)
{
	char *bin_data = NULL;
	ButtonCode_handle *buttoncode;
	char *up_down;
	...
	buttoncode = wndDpt->code_handle;
	if(wndDpt->buttonStatus ==1) {
		up_down = "up";
	} else {
		up_down = "down";
	}
	while(buttoncode != NULL) {
		if(SLIB_strcmp(buttoncode->up_down, up_down) == 0){
			bin_data = buttoncode->datas;
			delay = SLIB_atoi(buttoncode->delay);
			//__msg("data=%s\n",buttoncode->datas);
		}
		buttoncode = buttoncode->next;
	}
	xml_Lamp_Sent_gzq(wndDpt->button_handle, bin_data, delay);
}
