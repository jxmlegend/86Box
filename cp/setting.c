...
static __u32 system_update(void)
{
	...
	if(file){
		...
		eLIBs_fclose(file);
		h_lyr = UpdateWndCreate(320, 240,ID_CLOCK_UPDATE_BMP);
		dsk_fw_update("f:\\ePDKv100.img",h_lyr);
	}else{
		h_lyr = UpdateWndCreate(320, 240,ID_CLOCK_SD_BMP);
		esKRNL_TimeDly(50);
	}
...
static __s32 settingupdateWndProc(__gui_msg_t * msg)
{
	...
	case GUI_MSG_CREATE:
		...
		wndDpt->on = dsk_Systheme_open(ID_CLOCK_UPDATE_SYSC_BMP);
		wndDpt->ondata = dsk_Systheme_hdl2buf(wndDpt->on);
		wndDpt->off = dsk_Systheme_open(ID_CLOCK_UPDATE_SYSO_BMP);
...
static __s32 settingTopWndProc(__gui_msg_t * msg)
{
	...
	case GUI_MSG_CREATE:
		...
		wndDpt->ewii.gestureArgs.hmsg_x[GG_GESTURE_MSG_DEST_CLICK] = msg->h_deswin;
		wndDpt->ewii.gestureArgs.hmsg_x[GG_GESTURE_MSG_DEST_MO] = msg->h_deswin;
		wndDpt->on = dsk_Systheme_open(ID_CLOCK_RES_O_BMP);
		wndDpt->ondata = dsk_Systheme_hdl2buf(wndDpt->on);
		wndDpt->off = dsk_Systheme_open(ID_CLOCK_RES_C_BMP);
		wndDpt->offdata= dsk_Systheme_hdl2buf(wndDpt->off);
		break;
	case GUI_MSG_DESTROY:
		dsk_theme_close(wndDpt->on);
...
static __s32 settingBottomWndProc(__gui_msg_t * msg)
{
    ...
    case GUI_MSG_CREATE:
        ...
        wndDpt->ewii.hWnd = msg->h_deswin;
        wndDpt->bg = dsk_Systheme_open(ID_CLOCK_UPDATE_BG_BMP);
    ...
    case GUI_MSG_PAINT:
		...
		GUI_DispStringAt("A330_20140810.v1.0",90,210);
        break;
