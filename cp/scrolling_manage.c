...
#define MW_SYS_FONT_PATH2		"d:\\res\\fonts\\fangsong_20.sft"
    ...
    case GUI_MSG_PAINT:
		...
		if(wndDpt->backgroundData){
	        ...
	        GUI_FillRect(wndDpt->scrolling.ewii.wndRect.x + wndDpt->scrolling.wndRect.x,wndDpt->scrolling.ewii.wndRect.y + wndDpt->scrolling.wndRect.y,
	            wndDpt->scrolling.ewii.wndRect.x + wndDpt->scrolling.wndRect.x + 320, wndDpt->scrolling.ewii.wndRect.y + wndDpt->scrolling.wndRect.y + 240);
		}
        break;
    default:
#ifdef THIS_DEBUG
...
static __s32 scrollingButtonProc(__gui_msg_t * msg)
{
    ...
    case GUI_MSG_CREATE:
	   ...
	   wndDpt->dev_id = GG_DOMGetNodeValue(GG_DOMGetBrotherElementByTagName(firstNode, "dev_id"));
	   wndDpt->dev_type = GG_DOMGetNodeValue(GG_DOMGetBrotherElementByTagName(firstNode, "dev_type"));
	   node = GG_DOMGetBrotherElementByTagName(firstNode, "key_code");
	   wndDpt->key_code = SLIB_atoi(GG_DOMGetNodeValue(node));
	   wndDpt->button_handle = msg->h_deswin;
	...
	case GUI_MSG_USER_GESTURE_MOVE:
		eLIBs_printf("GUI_MSG_USER_GESTURE_MOVE\n");
	 ...
	 case GUI_MSG_USER_GESTURE_DONE:
	 	eLIBs_printf("GUI_MSG_USER_GESTURE_DONE\n");
	 	if(wndDpt->buttonStatus_close == 1){
			break;
		}
		if(SLIB_strcmp(wndDpt->xml_button_controltype,"slider") == 0){
	...
	case GUI_MSG_USER_GESTURE_MO:
		eLIBs_printf("GUI_MSG_USER_GESTURE_MO\n");
	...
	case GUI_MSG_USER_GESTURE_PAINT_NEIGHBOR:
		eLIBs_printf("GUI_MSG_USER_GESTURE_PAINT_NEIGHBOR\n");
		if(wndDpt->buttonStatus_close == 1){
			break;
		}
	...
	case GUI_MSG_USER_GESTURE_CLICK:
		eLIBs_printf("GUI_MSG_USER_GESTURE_CLICK\n");
		//receive_wrong(320,240,ID_CLOCK_TONGXINZHONG_BMP);
		//break;
	   R_red_write = 0;
			...
			if(wndDpt->buttonStatus_close == 0){
			 ...
			 if(wndDpt->page >= 0){
				 ...
				 if(if_top_down_move == 0){
					 ...
					 msgsend.dwAddData2 = MAKELONG(wndDpt->page * 320,100);
				 } else if(if_top_down_move == 1){
					 if(wndDpt->page > Page_Number){
						 wndDpt->page = 1;
					 }
					 msgsend.dwAddData2 = MAKELONG(wndDpt->page * 240,100);
				 }
    ...
    case GUI_MSG_USER_GESTURE_LPRESS:
		eLIBs_printf("GUI_MSG_USER_GESTURE_LPRESS\n");
		if(SLIB_strcmp(wndDpt->xml_button_controltype,"press") == 0){
		     //dsk_keytone_on();
		     wndDpt->xml_button_controltype = "press";
		     wndDpt->buttonStatus = 1;
	...
	case GUI_MSG_USER_GESTURE_CLICK_MSG://点击处理
		eLIBs_printf("GUI_MSG_USER_GESTURE_CLICK_MSG\n");
	...
	case GUI_MSG_USER_GESTURE_CONTRAL://点击按键的改变
		eLIBs_printf("GUI_MSG_USER_GESTURE_CONTRAL\n");
		father_xml_id_relation = wndDpt->xml_button_controltype;
	...
	case GUI_MSG_USER_GESTURE_XIA_MSG://下传处理
		eLIBs_printf("GUI_MSG_USER_GESTURE_XIA_MSG\n");
	...
	case GUI_MSG_USER_GESTURE_SHANG_MSG:
		eLIBs_printf("GUI_MSG_USER_GESTURE_SHANG_MSG\n");
	...
	case GUI_MSG_USER_GESTURE_RED_ZHUANGTAI:
		eLIBs_printf("GUI_MSG_USER_GESTURE_RED_ZHUANGTAI\n");
		 R_red_write = 1;
		 if(SLIB_strcmp(wndDpt->xml_button_controltype,"onoff") == 0){
	         wndDpt->xml_button_controltype = "onoff";
			 if(wndDpt->code_handle){
	...
	case GUI_MSG_USER_GESTURE_CHANGE_ZHUANGTAI:
		eLIBs_printf("GUI_MSG_USER_GESTURE_CHANGE_ZHUANGTAI\n");
		 if(SLIB_strcmp(wndDpt->xml_button_controltype,"onoff") == 0){
			 wndDpt->xml_button_controltype = "onoff";
			 if(msg->dwAddData2 <= 1){
