...
win_huadongtiao_handle * XML_haudongtiao_WinHandle_Search_gzq(char *dev_id, char *dev_type, int key_code)
{
	SCR_ScrollingButtonDescriptor * wndDpt;
	win_huadongtiao_handle *handle = xml_huadongtiao_win_handle->next;
	int i = 0;
	
	while(handle){
		wndDpt = (SCR_ScrollingButtonDescriptor *)GUI_WinGetAttr(handle->need_handle);
		...
		if((SLIB_strcmp(wndDpt->dev_id, dev_id) == 0) \
			&& (SLIB_strcmp(wndDpt->dev_type, dev_type) == 0) \
			&& (wndDpt->key_code == key_code)) {
			return handle;
		}
		handle = handle->next;
	}
