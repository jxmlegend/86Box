...
static void set_chushizhi(char * device_id)
{
	unsigned char * payload;
	int len;

	len = eLIBs_strlen(device_id);
	if(len > 0){
		if(device_id[1] == '.'){
			Work_SheBei_id = Tong_Xin_ToInt(device_id[0]) * 4;
			ChuanKo_id = Tong_Xin_ToInt(device_id[2]);
			__msg("%d,%d\n",Work_SheBei_id,ChuanKo_id);
		}else if(device_id[2] == '.'){
			Work_SheBei_id = (Tong_Xin_ToInt(device_id[0])*10 + Tong_Xin_ToInt(device_id[1])) * 4;
			ChuanKo_id = Tong_Xin_ToInt(device_id[3]) * 10 + Tong_Xin_ToInt(device_id[4]);
			__msg("%d,%d\n",Work_SheBei_id,ChuanKo_id);
		}
	}else{
		Work_SheBei_id = ChuanKo_id = 0;
	}
}
static __s32 scrollingHostWndProc(__gui_msg_t * msg)
{
    ...
    case GUI_MSG_CREATE:
		...
		device_id = GG_DOMGetNodeValue(GG_DOMGetBrotherElementByTagName(page, "deviceid"));
		//LogMI("device_id == %s",device_id);
		set_chushizhi(device_id);
		if(page){
			...
			//__msg("lanid=%d\n",lang);
			esDEV_Plugin("\\drv\\uart.drv", 1, (void *)9600, 0);
			/*if(inter_face){
...
#if 0
		if(wndDpt->geatrue_direction == 1){
			if(wndDpt->clock_en == 1){
				if(page_now != (LOSWORD(msg->dwAddData2) / 320 - 1)){
					XML_haudongtiao_Get_PageHandle(LOSWORD(msg->dwAddData2) / 320 - 1);
					page_now = (LOSWORD(msg->dwAddData2) / 320 - 1);
				}
			}else{
				if(page_now != (LOSWORD(msg->dwAddData2) / 320)){
					XML_haudongtiao_Get_PageHandle(LOSWORD(msg->dwAddData2) / 320);
					page_now = (LOSWORD(msg->dwAddData2) / 320);
				}
			}
		}else if(wndDpt->geatrue_direction == 2){
			if(wndDpt->clock_en == 1){
				if(page_now != (LOSWORD(msg->dwAddData2) / 240 - 1)){
					XML_haudongtiao_Get_PageHandle(LOSWORD(msg->dwAddData2) / 240 - 1);
					page_now = (LOSWORD(msg->dwAddData2) / 240 - 1);
				}
			}else{
				if(page_now != (LOSWORD(msg->dwAddData2) / 240)){
					XML_haudongtiao_Get_PageHandle(LOSWORD(msg->dwAddData2) / 240);
					page_now = (LOSWORD(msg->dwAddData2) / 240);
				}
			}
		}
#endif
