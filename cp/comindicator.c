...
static __s32 IndicatorWndProc(__gui_msg_t * msg)
{
    ...
    case GUI_MSG_CREATE:
        ...
        desc->on = dsk_Systheme_open(ID_CLOCK_COM_PAGE_S_BMP);
        desc->onData = dsk_Systheme_hdl2buf(desc->on);
        desc->off = dsk_Systheme_open(ID_CLOCK_COM_PAGE_BMP);
        desc->offData = dsk_Systheme_hdl2buf(desc->off);
